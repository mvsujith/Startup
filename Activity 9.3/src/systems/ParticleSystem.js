...
export class ParticleSystem {
    constructor(scene) {
        ...
        gltfLoader.load('/building.glb', (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.isMesh) {
                    this.buildingGeometry = child.geometry.clone();
                    // Optional: adjust scale/rotation if needed from the model
                }
            });
            console.log("✅ building.glb geometry loaded");
        });

        // Load Glass Jar GLB
        this.glassJarGeometry = null;
        gltfLoader.load('/glass_jar.glb', (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.isMesh) {
                    this.glassJarGeometry = child.geometry.clone();
                }
            });
            console.log("✅ glass_jar.glb geometry loaded");
        });

        // Load Paper GLB
        this.paperGeometry = null;
        gltfLoader.load('/paper.glb', (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.isMesh) {
                    this.paperGeometry = child.geometry.clone();
                }
            });
            console.log("✅ paper.glb geometry loaded");
        });

        // Load Stone GLB
        this.stoneGeometry = null;
        gltfLoader.load('/stone.glb', (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.isMesh) {
                    this.stoneGeometry = child.geometry.clone();
                }
            });
            console.log("✅ stone.glb geometry loaded");
        });
    }
    ...
    createSettlingParticles(material, beakerObject, waterLevel) {
        const particleCount = material.particleCount;
        let geometry;
        if (material.name === 'Building') {
            if (this.buildingGeometry) {
                geometry = this.buildingGeometry.clone();
                ...
                console.warn("⚠️ Building GLB not loaded yet, using fallback box");
                geometry = new THREE.BoxGeometry(material.particleSize * 0.5, material.particleSize, material.particleSize * 0.5);
            }
        } else if (material.name === 'Glass Jar') {
            if (this.glassJarGeometry) {
                geometry = this.glassJarGeometry.clone();
                geometry.scale(material.particleSize, material.particleSize, material.particleSize);
            } else {
                geometry = new THREE.CylinderGeometry(material.particleSize * 0.5, material.particleSize * 0.5, material.particleSize * 1.0, 16);
            }
        } else if (material.name === 'Paper') {
            if (this.paperGeometry) {
                geometry = this.paperGeometry.clone();
                geometry.scale(material.particleSize, material.particleSize, material.particleSize);
            } else {
                geometry = new THREE.PlaneGeometry(material.particleSize, material.particleSize * 1.4);
            }
        } else if (material.name === 'Stone') {
            if (this.stoneGeometry) {
                geometry = this.stoneGeometry.clone();
                geometry.scale(material.particleSize, material.particleSize, material.particleSize);
            } else {
        ...
        const particleMaterial = new THREE.MeshStandardMaterial({
            ...
            color: material.color,
            metalness: material.name === 'Building' ? 0.1 : 0.1,
            roughness: material.name === 'Building' ? 0.8 : 0.9,
        });
