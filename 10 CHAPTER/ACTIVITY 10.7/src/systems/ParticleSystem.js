...
export class ParticleSystem {
    constructor(scene) {
        ...
        gltfLoader.load('/Table.glb', (gltf) => {
            gltf.scene.traverse((child) => {
                if (child.isMesh) {
                    this.tableGeometry = child.geometry.clone();
                    // Optional: adjust scale/rotation if needed from the model
                }
            });
            console.log("✅ Table.glb geometry loaded");
        });
    }
    ...
    createSettlingParticles(material, beakerObject, waterLevel) {
        const particleCount = material.particleCount;
        let geometry;
        if (material.name === 'Table') {
            if (this.tableGeometry) {
                geometry = this.tableGeometry.clone();
                ...
                console.warn("⚠️ Table GLB not loaded yet, using fallback box");
                geometry = new THREE.BoxGeometry(material.particleSize * 0.5, material.particleSize * 0.5, material.particleSize * 0.5);
            }
        } else if (material.name === 'Trolly') {
            geometry = new THREE.BoxGeometry(material.particleSize, material.particleSize * 0.5, material.particleSize * 1.5);
        } else if (material.name === 'String') {
            geometry = new THREE.CylinderGeometry(material.particleSize * 0.1, material.particleSize * 0.1, material.particleSize * 2, 8);
            geometry.rotateX(Math.PI / 2);
            geometry.rotateZ(Math.random() * Math.PI);
        } else if (material.name === 'Pulley') {
            geometry = new THREE.CylinderGeometry(material.particleSize, material.particleSize, material.particleSize * 0.5, 16);
            geometry.rotateX(Math.PI / 2);
        } else if (material.name === 'Slotted Weights') {
            geometry = new THREE.CylinderGeometry(material.particleSize, material.particleSize, material.particleSize * 0.5, 16);
            geometry.rotateX(Math.PI / 2);
        } else if (material.name === 'Wooden Block') {
            geometry = new THREE.BoxGeometry(material.particleSize, material.particleSize, material.particleSize);
        } else if (material.name === 'Scale') {
            geometry = new THREE.BoxGeometry(material.particleSize, material.particleSize * 0.2, material.particleSize);
        } else {
        ...
        const particleMaterial = new THREE.MeshStandardMaterial({
            ...
            color: material.color,
            metalness: material.name === 'Table' ? 0.1 : 0.1,
            roughness: material.name === 'Table' ? 0.8 : 0.9,
        });
